...
export function ProjectDashboard({ nodes, edges, architecture }: ProjectDashboardProps) {
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div className={cn(tile, "flex flex-col gap-4 p-5")}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-200">Umsetzungs-Roadmap</span>
          </div>
          <ol className="flex flex-col gap-0">
            {(architecture.roadmap ?? []).map((step, i) => (
              <li key={i} className="flex gap-3">
                <div className="flex flex-col items-center">
                  <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-amber-400/40 bg-amber-500/15 text-xs font-bold text-amber-300">
                    {i + 1}
                  </div>
                  {i < (architecture.roadmap?.length ?? 0) - 1 && (
            ...
            {!architecture.roadmap?.length && (
              <p className="text-xs text-slate-500">Keine Roadmap verfügbar.</p>
            )}
          </ol>
        </div>
        ...
        <div className={cn(tile, "flex flex-col gap-4 p-5")}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-200">Lernressourcen</span>
          </div>
          <ul className="flex flex-col gap-2">
            {(architecture.learningResources ?? []).map((resource, i) => (
              <li
                key={i}
                className="flex items-start gap-2 rounded-xl border border-white/6 bg-white/3 px-3 py-2.5"
              >
            ...
            {!architecture.learningResources?.length && (
              <p className="text-xs text-slate-500">Keine Ressourcen verfügbar.</p>
            )}
          </ul>
        </div>
