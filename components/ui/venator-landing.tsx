      ...
      <div className="relative z-10 w-full">
        <nav className="w-full px-6 py-4">
            <div className="max-w-7xl mx-auto flex justify-between items-center">
                ...
                <button onClick={actionButton.onClick} className="px-4 py-2 rounded-lg text-foreground text-sm font-medium border border-border bg-background/50 backdrop-blur-md hover:bg-accent hover:text-accent-foreground transition-colors">{actionButton.label}</button>
            </div>
        </nav>
            ...
            <div className="max-w-6xl mx-auto text-center">
                <div className="mb-8 float-animation">
                    <h1 className="md:text-6xl lg:text-7xl leading-[1.1] text-5xl font-light text-foreground tracking-tight mb-4">
                        {hero.titleLine1}
                        <span className="block tracking-tight font-medium bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-emerald-400">{hero.titleLine2Gradient}</span>
                    </h1>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
                    <button onClick={ctaButtons.primary.onClick} className="bg-foreground text-background hover:bg-foreground/90 px-6 py-3 rounded-lg font-medium text-sm min-w-[160px] transition-colors shadow-lg">{ctaButtons.primary.label}</button>
                    <button onClick={ctaButtons.secondary.onClick} className="min-w-[160px] text-sm font-medium text-foreground rounded-lg px-6 py-3 border border-border backdrop-blur-sm bg-background/30 hover:bg-accent/50 transition-colors">{ctaButtons.secondary.label}</button>
                </div>
                <div className="mb-16 h-px w-full bg-border/50" />
                <div id="features" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto mb-16">
                    {features.map((feature, index) => (
                        <div key={index} className="rounded-2xl p-6 text-left border border-border bg-card/30 backdrop-blur-md hover:bg-card/60 transition-colors shadow-sm">
                            <div className="rounded-xl h-32 mb-4 flex items-center justify-center bg-muted/50 overflow-hidden">{feature.imageContent}</div>
                            <h3 className="text-lg font-medium text-card-foreground mb-2">{feature.title}</h3>
                            <p className="text-muted-foreground text-sm mb-4">{feature.description}</p>
                            <div className="flex flex-wrap gap-2">
                                {feature.tags.map(tag => (
                                    <span key={tag} className="px-2 py-1 rounded text-xs text-muted-foreground bg-secondary/80 border border-border/50">{tag}</span>
                                ))}
                            </div>
                        </div>
                    ))}
