...
- **Naming Conventions**: Component files must use PascalCase (e.g., `ModernAnimatedSignIn.tsx`); Utility files use camelCase.
- **Client vs Server**: Server Components by default; only use `'use client'` when React hooks or interactivity are necessary.
- **Forms & Data**: Prefer Server Actions for form submissions. Use Zod for validation.
- **Language**: All code, variables, and technical comments must be in English. All user-facing UI texts (labels, placeholders, text) MUST be in German.

Determine the default path for components and styles in the project. Place the main component blocks in `components/ui/` or `components/blocks/`.
Copy-paste and adapt this component code to our conventions:
```tsx
...
export function Demo() {
  ...
  const formFields = {
    ...
    subHeader: 'Melden Sie sich bei Ihrem Konto an',
    fields: [
      {
        label: 'E-Mail',
        required: true,
    ...
    textVariantButton: 'Passwort vergessen?',
  };
...
Extend existing `app/globals.css` with the required Tailwind CSS v4 configuration and variables (Note: Tailwind v4 uses CSS variables, not `tailwind.config.js`):
```css
@theme {
...
Implementation Guidelines
 1. Analyze the component structure and convert it to strictly follow our Next.js 15, TypeScript, and Tailwind v4 setup.
 2. Use Zod and React Hook Form (or Server Actions) to handle the form logic for this login/signup component.
 3. Ensure all text and placeholders displayed to the user are translated to German. Internal code and variables stay in English.
 4. Identify any required context providers, like theme providers, and ensure they are used appropriately.

Steps to integrate
 0. Create the component files using PascalCase inside `components/ui/` or `components/blocks/`.
 1. Install any missing external dependencies (e.g., `motion`, `lucide-react`).
 2. Adjust the code to use `'use client'` only when strictly necessary, and extract server-side logic (e.g. form submission) into Server Actions.
